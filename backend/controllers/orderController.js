...
const generateOrderNumber = async () => {
  ...
  if (latestOrder?.orderNumber) {
    const lastPart = latestOrder.orderNumber.split("-").pop();
...
const buildOrderItems = async (items) => {
  ...
  for (const item of items) {
    ...
    if (quantity > 100) {
      return {
        error: `Maximum quantity for ${product.name} is 100`,
      };
    }
    ...
    if (taxRate < 0 || taxRate > 100) {
      return {
        error: `Invalid tax rate configured for ${product.name}`,
      };
    }
...
export const createCustomerOrder = async (req, res) => {
  try {
    const customerDocument = await resolveAuthenticatedCustomer(req.user);
    ...
    if (customerDocument.isActive === false) {
    ...
    const itemResult = await buildOrderItems(items);
    ...
    const { orderItems, subtotal, itemTaxAmount } = itemResult;

    const discount = 0;

    const finalTax = roundMoney(itemTaxAmount);

    const totalAmount = roundMoney(subtotal - discount + finalTax);
    ...
    const populatedOrder = await Order.findById(order._id)
      .populate("customer", "name phone email customerType isActive")
      .populate("createdBy", "name email role")
      .populate("items.product", "name image sku price taxRate unit");
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to create online order",
    });
  }
};
  ...
  try {
    ...
    if (req.user.role === "customer") {
      if (orderType !== "online") {
        return res.status(400).json({
          success: false,
          message: "Customers can only create online orders",
        });
      }

      if (!["razorpay", "unpaid"].includes(paymentMethod)) {
        return res.status(400).json({
          success: false,
          message: "Customers must use Razorpay for online orders",
        });
      }

      customerDocument = await resolveAuthenticatedCustomer(req.user);
      ...
      if (customer) {
        ...
        customerDocument = await Customer.findById(customer);
    ...
    if (!["pos", "online"].includes(orderType)) {
      return res.status(400).json({
        success: false,
    ...
    if (discount > 0 && subtotal > 0) {
      ...
      finalTax = roundMoney(itemTaxAmount * (taxableBase / subtotal));
    }
    ...
    const customerSnapshot = customerDocument
      ? {
          name: customerDocument.name,
    ...
    const order = await Order.create({
      ...
      customer: customerDocument?._id || null,
...
export const getOrders = async (req, res) => {
  try {
    ...
    if (req.user.role === "customer") {
      ...
      filter.customer = customerDocument._id;
    } else if (customer) {
      if (!isValidObjectId(customer)) {
        return res.status(400).json({
          success: false,
...
export const getOrderById = async (req, res) => {
  try {
    ...
    if (req.user.role === "customer") {
      ...
      if (
        !order.customer ||
        order.customer._id.toString() !== customerDocument._id.toString()
      ) {
...
export const updateOrderStatus = async (req, res) => {
  try {
    ...
    if (order.status === "refunded" && status !== "refunded") {
      return res.status(400).json({
        success: false,
        message: "A refunded order cannot be reactivated",
      });
    }

    if (
      order.status === "completed" &&
      ["draft", "pending", "confirmed", "processing"].includes(status)
    ) {
      return res.status(400).json({
        success: false,
        message: "A completed order cannot be moved back to an earlier status",
      });
    }
  ...
  try {
    ...
    return res.status(200).json({
      ...
      stats: {
        ...
        totalRevenue: roundMoney(revenue.totalRevenue),

        totalTax: roundMoney(revenue.totalTax),

        totalDiscount: roundMoney(revenue.totalDiscount),
      },
    });
  } catch (error) {
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve order statistics",
    });
  }
};
