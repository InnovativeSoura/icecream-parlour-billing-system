...
export const getInventory = async (req, res) => {
  try {
    ...
    inventory.forEach((item) => {
      inventoryMap.set(String(item.product?._id), item);
    });

    let result = products.map((product) => {
      const item = inventoryMap.get(String(product._id));
      ...
      const lowStockThreshold =
        item?.lowStockThreshold ?? product.lowStockThreshold ?? 10;
      ...
      return {
        ...
        lastStockUpdateAt: item?.lastStockUpdateAt || null,
...
export const getInventoryByProduct = async (req, res) => {
  try {
    ...
    let inventory = await Inventory.findOne({
      product: productId,
    }).populate("product", "name sku price unit image");
...
export const adjustStock = async (req, res) => {
  try {
    ...
    inventory.lastStockUpdateAt = new Date();
...
export const getStockMovements = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Unable to retrieve stock movements",
    });
  }
};
